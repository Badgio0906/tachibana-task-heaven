...
func hud() -> void:
	...
	box(Rect2(269, 67, maxf(7, 426 * game.scoring.task / 100), 26), TEAL if game.scoring.task >= 70 else ORANGE, 13)
	draw_line(Vector2(567, 63), Vector2(567, 99), INK, 2)
	txt("70%", 549, 116, 12)
	txt("%d COMBO" % game.scoring.combo, 731, 71, 30)
	txt("MISS  %d / 15" % game.scoring.misses, 732, 104, 17)
...
func help() -> void:
	...
	txt("PERFECT ±90ms / GOOD ±180ms / OK ±280ms", 128, 430, 18)
	txt("最後にTASK 70%以上でクリア。", 128, 468, 21)
	txt("15 MISSでカムチャッカ行き！", 128, 505, 21)
