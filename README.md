...
初回STARTで説明と練習に進みます。上司のお手本の後、「入力まであと2拍→1拍→今！」の合図に合わせて同じリズムで入力してください。休符は押さずに待ちます。Escまたは右上のⅡで一時停止。音の調整はタイトルの「遊び方 / 音の調整」から。

覚えるのが難しいときは、説明画面か一時停止画面で **OJTモード**をONにしてください。次に押す番号と画面を1拍前から表示します。判定の甘さや得点は変わりません。設定は次回も保持されます。
...
- 保存先：このゲーム専用の `user://task_heaven.cfg`。練習済み、ハイスコア、音量、入力補正、OJTモード。
...
| 画面構成とキャラクター演出 | `scripts/office_view.gd` / `scripts/worker_overlay.gd` / `scripts/task_screen.gd` |
